...
public enum DeterminationGenerator {
    ...
    /// Internal function to implement the core determine basal logic.
    static func determineBasal(
        ...
        // Safety: if we ever get an invalid Decimal, handle
    ...
    static func handleTempBasalCases(
        ...
        // Compose tick for log (kept for iOS parity)
        _ = (delta > -0.5) ? "+\(delta.rounded(toPlaces: 0))" : "\(delta.rounded(toPlaces: 0))"
        let minDelta = min(delta, shortAvgDelta)
        _ = min(shortAvgDelta, longAvgDelta) // minAvgDelta - kept for iOS parity
        _ = max(delta, shortAvgDelta, longAvgDelta) // maxDelta - kept for iOS parity
